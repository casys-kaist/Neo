...
void global_sorter_t::tick_idle() {
    if (m_ready) {
        ...
        if (num_gaussian != 0) {
            ...
            int max_level = ceil(log2(num_gaussian / to_u64(get_config("OTHER", "GlobalChunkSize"))));
...
adaptive_sorter_t::adaptive_sorter_t() {
    m_ready = false;
    m_ready_tile_id = 0;
    m_remain_gaussian = 0;

    m_state = ADAPTIVE_SORTER_STATE_IDLE;
...
void adaptive_sorter_t::tick_idle() {
    if (m_ready) {
        m_ready = false;
        m_state = ADAPTIVE_SORTER_STATE_RUN;

        m_remain_gaussian = g_trace->trace.num_reuse_gaussian_per_tile[m_ready_tile_id];

        if (m_remain_gaussian != 0) {
            const u64 chunk_size = to_u64(get_config("OTHER", "AdaptiveChunkSize"));

            m_load_bit = true;
            m_load_chunk_size = min(m_remain_gaussian, chunk_size);
            m_load_id = g_dram_wrapper->continuous_dram_access(reuse_tile_base(m_ready_tile_id),
                                                               m_load_chunk_size * REUSE_ENTRY_SIZE,
                                                               false,
                                                               0,
                                                               "Adaptive Sort (Load)");
            m_load_tile_id = m_ready_tile_id;
            m_remain_gaussian -= m_load_chunk_size;
            m_load_end = m_remain_gaussian == 0;
        } else {
            g_reuse_finish[m_ready_tile_id] = true;
            if (g_reuse_finish[m_ready_tile_id] && g_common_finish[m_ready_tile_id])
                g_render_queue.push(m_ready_tile_id);
        }
    }
}

void adaptive_sorter_t::tick_run() {
    bool all_done = true;

    if (m_load_bit && !g_dram_wrapper->is_finished(m_load_id))
        all_done = false;

    if (m_sort_bit && m_sort_remain_cycle > 0)
        all_done = false;
    ...
    if (all_done) {
        if (!m_load_bit && !m_sort_bit && !m_store_bit)
            m_state = ADAPTIVE_SORTER_STATE_IDLE;
        else {
            if (m_store_bit && g_dram_wrapper->is_finished(m_store_id)) {
                if (m_store_end) {
                    g_reuse_finish[m_store_tile_id] = true;
                    if (g_reuse_finish[m_store_tile_id] && g_common_finish[m_store_tile_id])
                        g_render_queue.push(m_store_tile_id);
                }
            ...
            if (m_sort_bit && m_sort_remain_cycle == 0) {
                ...
                m_store_bit = true;
                m_store_end = m_sort_end;
                m_store_tile_id = m_sort_tile_id;
                m_store_chunk_size = m_sort_chunk_size;
                m_store_id = g_dram_wrapper->continuous_dram_access(reuse_tile_base(m_store_tile_id),
            ...
            if (m_load_bit && g_dram_wrapper->is_finished(m_load_id)) {
                ...
                m_sort_bit = true;
                m_sort_end = m_load_end;
                m_sort_tile_id = m_load_tile_id;
                m_sort_chunk_size = m_load_chunk_size;
                const u64 sort_granularity = to_u64(get_config("OTHER", "SortGranularity"));
                const u64 merge_sort_max_depth = (m_sort_chunk_size <= sort_granularity) ? 1 : ceil(log2(m_sort_chunk_size / sort_granularity));
                m_sort_remain_cycle = merge_sort_max_depth * m_sort_chunk_size;
            }
            ...
            if (m_remain_gaussian > 0) {
                ...
                const u64 chunk_size = to_u64(get_config("OTHER", "AdaptiveChunkSize"));
                m_load_chunk_size = min(m_remain_gaussian, chunk_size);
                m_load_id = g_dram_wrapper->continuous_dram_access(reuse_tile_base(m_load_tile_id),
                                                                   m_load_chunk_size * REUSE_ENTRY_SIZE,
                                                                   false,
                                                                   0,
                                                                   "Adaptive Sort (Load)");
...
void adaptive_sorter_t::tick() {
    m_sort_remain_cycle = (m_sort_remain_cycle > 0) ? m_sort_remain_cycle - 1 : 0;

    switch (m_state) {
    case ADAPTIVE_SORTER_STATE_IDLE:
        tick_idle();
        break;
    case ADAPTIVE_SORTER_STATE_RUN:
        tick_run();
        break;
    }
}

void adaptive_sorter_t::ready(u64 tile_id) {
    m_ready = true;
    m_ready_tile_id = tile_id;
}

bool adaptive_sorter_t::can_ready() {
    if (m_ready)
        return false;
...
bool adaptive_sorter_t::is_finished() {
    if (m_ready)
        return false;

    if (m_state == ADAPTIVE_SORTER_STATE_RUN)
        return false;
