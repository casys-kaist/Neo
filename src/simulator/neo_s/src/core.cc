...
void core_t::tick_parallel() {
    ...
    if (!m_reuse_phase.is_finished())
        all_done = false;

    if (!m_render_phase.is_finished())
        all_done = false;

    if (all_done) {
        m_state = CORE_STATE_POSTPROCESSING;
        m_postprocessor.ready();
    }
}

void core_t::tick_postprocessing() {
    if (m_postprocessor.is_finished()) {
        spdlog::info("Postprocessing Phase Finish");
        m_state = CORE_STATE_IDLE;
    } else {
        m_postprocessor.tick();
    }
}
...
void core_t::tick() {
    ...
    case CORE_STATE_PARALLEL:
        tick_parallel();
        break;
    case CORE_STATE_POSTPROCESSING:
        tick_postprocessing();
        break;
    }
}
