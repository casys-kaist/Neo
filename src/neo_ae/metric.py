...
def measure_PSNR(img1, img2):
    mse = ((img1 - img2) ** 2).view(img1.shape[0], -1).mean(1, keepdim=True)
    return (20 * torch.log10(1.0 / torch.sqrt(mse))).item()


def measure_SSIM(img1, img2, window_size=11, size_average=True):
    channel = img1.size(-3)
...
def measure(img1_path, img2_path, metric):
    read_image = (
        lambda file_path: tf.to_tensor(Image.open(file_path))
        .unsqueeze(0)[:, :3, :, :]
        .cuda()
    )
    ...
    img2 = transforms.Resize(img1.shape[-2:])(img2)

    if metric == "PSNR":
        return measure_PSNR(img1, img2)
    elif metric == "SSIM":
        return measure_SSIM(img1, img2)
    elif metric == "LPIPS":
        return measure_LPIPS(img1, img2)
    else:
